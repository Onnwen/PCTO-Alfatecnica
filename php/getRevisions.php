...
# TODO: Bisogna implementare la data di scadenza variabile; la Deadline non sarà sempre a 6 mesi dall'ultima revisione
$Query = "SELECT Companies.name as CompanyName, Product_Category.name as ProductCategoryName, Tabella.LastRevision, ADD_MONTHS(Tabella.LastRevision, 6) as Deadline FROM Companies, Product_Category, (SELECT Revisions.product_category_id, Revisions.company_id, MAX(DATE(Revisions.data)) as LastRevision FROM Revisions GROUP BY Revisions.company_id, Revisions.product_category_id) as Tabella WHERE Tabella.product_category_id = Product_Category.product_category_id AND Tabella.company_id = Companies.id ORDER BY Deadline ASC";
